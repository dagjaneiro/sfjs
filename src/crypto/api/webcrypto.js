...
export class SFCryptoWeb extends SFAbstractCrypto {
  ...
  */
  ...
  async generateRandomKey(bits) {
    ...
    return subtleCrypto
      .generateKey({ name: 'AES-CBC', length: bits }, extractable, [
        'encrypt',
        'decrypt'
      ])
      .then(keyObject => {
        return subtleCrypto
          .exportKey('raw', keyObject)
          .then(async keyData => {
            var key = await this.arrayBufferToHexString(
  ...
  async decryptText(
    ...
    if (authHash) {
      ...
      if (authHash !== localAuthHash) {
        console.error(
          `Auth hash does not match, returning null. ${authHash} != ${localAuthHash}`
        );
        return null;
      }
    }
  ...
  async hmac256(message, key) {
    ...
    var keyData = await this.webCryptoImportKey(keyHexData, 'HMAC', ['sign'], {
      name: 'SHA-256'
    });
    var messageData = await this.stringToArrayBuffer(message);
    return crypto.subtle
      .sign({ name: 'HMAC' }, keyData, messageData)
  ...
  */
  ...
  async webCryptoImportKey(input, alg, actions, hash) {
    var text =
      typeof input === 'string' ? await this.stringToArrayBuffer(input) : input;
    return subtleCrypto
      .importKey('raw', text, { name: alg, hash: hash }, false, actions)
      .then(key => {
        return key;
      })
      .catch(err => {
