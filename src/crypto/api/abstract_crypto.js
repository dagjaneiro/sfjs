...
export class SFAbstractCrypto {
  ...
  async decryptText(
    ...
    var decrypted = CryptoJS.AES.decrypt(contentCiphertext, keyData, {
      iv: ivData,
  ...
  async encryptText(text, key, iv) {
    ...
    var ivData = CryptoJS.enc.Hex.parse(iv || '');
    var encrypted = CryptoJS.AES.encrypt(text, keyData, {
      iv: ivData,
      mode: CryptoJS.mode.CBC,
      padding: CryptoJS.pad.Pkcs7
    });
  ...
  async generateSymmetricKeyPair({ password, pw_salt, pw_cost } = {}) {
    var output = await this.pbkdf2(
      password,
      pw_salt,
      pw_cost,
      this.DefaultPBKDF2Length
    );
  ...
  async computeEncryptionKeysForUser(password, authParams) {
    ...
    if (authParams.version === '003') {
      ...
      pw_salt = await this.generateSalt(
        authParams.identifier,
        authParams.version,
        authParams.pw_cost,
        authParams.pw_nonce
      );
    ...
    return this.generateSymmetricKeyPair({
      password: password,
      pw_salt: pw_salt,
