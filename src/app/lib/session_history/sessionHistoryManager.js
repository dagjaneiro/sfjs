...
export class SFSessionHistoryManager {
        ...
        (allItems, validItems, deletedItems, source, sourceKey) => {
          ...
          for (const item of allItems) {
            try {
              ...
              console.log(
                'Caught exception while trying to add item history entry',
                e
              );
            }
          }
  ...
  async loadFromDisk() {
    ...
    var historyValue = await this.storageManager.getItem(
      SessionHistoryRevisionsKey
    );
    if (historyValue) {
    ...
    var autoOptimizeValue = await this.storageManager.getItem(
      SessionHistoryAutoOptimizeKey
    );
