...
import { SFModelManager } from './modelManager';
import { SFItemParams } from '../models/itemParams';
import { SFHttpManager } from './httpManager';
...
export class SFSyncManager {
  ...
  async handleServerIntegrityHash(serverHash) {
    ...
    const localHash = await this.modelManager.computeDataIntegrityHash();
  ...
  async getServerURL() {
    return (
      (await this.storageManager.getItem('server')) || window._default_sf_server
    );
  }
  ...
  registerSyncStatusObserver(callback) {
    const observer = { key: new Date(), callback: callback };
  ...
  _sortLocalItems(items) {
    return items.sort((a, b) => {
      ...
      const priorityList = this.contentTypeLoadPriority;
      let aPriority = 0;
      let bPriority = 0;
      if (priorityList) {
  ...
  async loadLocalItems({ incrementalCallback, batchSize, options } = {}) {
    ...
    if (options && options.simulateHighLatency) {
      const latency = options.simulatedLatency || 1000;
    ...
    this.loadLocalDataPromise = this.storageManager
      .getAllModels()
      .then(items => {
        ...
        items = items.filter(nonDecryptedItem => {
          const localItem = this.modelManager.findItem(nonDecryptedItem.uuid);
          if (!localItem) {
            return true;
          }
        ...
        const decryptNext = async () => {
          const subitems = items.slice(current, current + batchSize);
          const processedSubitems = await this.handleItemsResponse(
            subitems,
            null,
            SFModelManager.MappingSourceLocalRetrieved,
            SFSyncManager.KeyRequestLoadLocal
          );
  ...
  async writeItemsToLocalStorage(items, offlineOnly) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const info = await this.getActiveKeyInfo(
        SFSyncManager.KeyRequestSaveLocal
      );

      if (nonDeletedItems.length > 0) {
        const params = await Promise.all(
          nonDeletedItems.map(async item => {
            let itemParams = new SFItemParams(
              item,
              info.keys,
              info.auth_params
            );
  ...
  async markAllItemsDirtyAndSaveOffline(alternateUUIDs) {
    if (alternateUUIDs) {
      ...
      const originalItems = this.modelManager.allNondummyItems
        .filter(item => {
    ...
    const allItems = this.modelManager.allNondummyItems;
  ...
  callQueuedCallbacks(response) {
    const allCallbacks = this.queuedCallbacks;
    if (allCallbacks.length) {
      for (const eachCallback of allCallbacks) {
  ...
  beginCheckingIfSyncIsTakingTooLong() {
    ...
    this.syncStatus.checker = this.$interval(
      function() {
        // check to see if the ongoing sync is taking too long, alert the user
        const secondsPassed = (new Date() - this.syncStatus.syncStart) / 1000;
        const warningThreshold = 5.0; // seconds
  ...
  async sync(options = {}) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const allDirtyItems = this.modelManager.getDirtyItems();
      const dirtyItemsNotYetSaved = allDirtyItems.filter(candidate => {
        return (
          !this.lastDirtyItemsSave ||
          candidate.dirtiedDate > this.lastDirtyItemsSave
        );
      });

      const info = await this.getActiveKeyInfo(
        SFSyncManager.KeyRequestLoadSaveAccount
      );

      const isSyncInProgress = this.syncStatus.syncOpInProgress;
      const initialDataLoaded = this.initialDataLoaded();
      ...
      if (isSyncInProgress || !initialDataLoaded) {
        ...
        if (isSyncInProgress) {
          ...
          if (this.loggingEnabled) {
            console.warn(
              'Attempting to sync while existing sync is in progress.'
            );
          }
        }
        if (!initialDataLoaded) {
          if (this.loggingEnabled) {
            console.warn(
              '(1) Attempting to perform online sync before local data has loaded'
            );
          }
      ...
      if (info.offline) {
        return this.syncOffline(allDirtyItems)
          .then(response => {
      ...
      if (!this.initialDataLoaded()) {
        console.error(
          'Attempting to perform online sync before local data has loaded'
        );
        return;
      }
      ...
      const isContinuationSync = this.syncStatus.needsMoreSync;
      this.syncStatus.syncStart = new Date();
      this.beginCheckingIfSyncIsTakingTooLong();

      const submitLimit = this.PerSyncItemUploadLimit;
      const subItems = allDirtyItems.slice(0, submitLimit);
      ...
      try {
        this.httpManager.postAuthenticatedAbsolute(
          await this.getSyncURL(),
          params,
          response => {
            this.handleSyncSuccess(subItems, response, options)
              .then(() => {
                resolve(response);
              })
              .catch(e => {
                console.log('Caught sync success exception:', e);
                this.handleSyncError(null, null, allDirtyItems).then(
                  errorResponse => {
  ...
  async handleSyncSuccess(syncedItems, response, options) {
    ...
    const allSavedUUIDs = this.allSavedItems.map(item => item.uuid);
    const currentRequestSavedUUIDs = response.saved_items.map(
      savedResponse => savedResponse.uuid
    );

    response.retrieved_items = response.retrieved_items.filter(
      retrievedItem => {
        const isInPreviousSaved = allSavedUUIDs.includes(retrievedItem.uuid);
        const isInCurrentSaved = currentRequestSavedUUIDs.includes(
          retrievedItem.uuid
        );
        ...
        const localItem = this.modelManager.findItem(retrievedItem.uuid);
    ...
    const retrieved = await this.handleItemsResponse(
    ...
    const saved = await this.handleItemsResponse(
      response.saved_items,
      omitFields,
      SFModelManager.MappingSourceRemoteSaved,
      SFSyncManager.KeyRequestLoadSaveAccount
    );
    ...
    const deprecated_unsaved = response.unsaved;
    await this.deprecated_handleUnsavedItemsResponse(deprecated_unsaved);

    const conflicts = await this.handleConflictsResponse(response.conflicts);

    const conflictsNeedSync = conflicts && conflicts.length > 0;
    if (conflicts) {
    ...
    if (response.integrity_hash && !response.cursor_token) {
      const matches = await this.handleServerIntegrityHash(
        response.integrity_hash
      );
      if (!matches) {
    ...
    const cursorToken = await this.getCursorToken();
    if (cursorToken || this.syncStatus.needsMoreSync) {
      return new Promise((resolve, reject) => {
        setTimeout(
          ...
          10
        ); // wait 10ms to allow UI to update
      });
      ...
      if (
        ...
        (deprecated_unsaved &&
          deprecated_unsaved.length >= this.majorDataChangeThreshold) ||
  ...
  async handleItemsResponse(responseItems, omitFields, source, keyRequest) {
    const keys = (await this.getActiveKeyInfo(keyRequest)).keys;
    ...
    const itemsWithErrorStatusChange = items.filter(item => {
      const valueChanged = item.errorDecryptingValueChanged;
  ...
  async refreshErroredItems() {
    const erroredItems = this.modelManager.allNondummyItems.filter(item => {
  ...
  */
  async handleConflictsResponse(conflicts) {
    ...
    for (const conflict of conflicts) {
      const serverItemResponse = conflict.server_item || conflict.unsaved_item;
      const localItem = this.modelManager.findItem(serverItemResponse.uuid);
      if (!localItem) {
        localValues[serverItemResponse.uuid] = {};
        continue;
      }
      const frozenContent = localItem.getContentCopy();
      localValues[serverItemResponse.uuid] = {
        frozenContent,
        itemRef: localItem
      };
    ...
    const itemsNeedingLocalSave = [];

    for (const conflict of conflicts) {
      // if sync_conflict, we receive conflict.server_item.
      // If uuid_conflict, we receive the value we attempted to save.
      const serverItemResponse = conflict.server_item || conflict.unsaved_item;
      await SFJS.itemTransformer.decryptMultipleItems(
        [serverItemResponse],
        (await this.getActiveKeyInfo(SFSyncManager.KeyRequestLoadSaveAccount))
          .keys
      );
      const { frozenContent, itemRef } = localValues[serverItemResponse.uuid];
      ...
      if (conflict.type === 'uuid_conflict') {
        ...
        const newItem = await this.modelManager.alternateUUIDForItem(itemRef);
        itemsNeedingLocalSave.push(newItem);
      } else if (conflict.type === 'sync_conflict') {
        const tempServerItem = await this.modelManager.createDuplicateItemFromResponseItem(
          serverItemResponse
        );
        // Convert to an object simply so we can have access to the `isItemContentEqualWith` function.
        const _tempItemWithFrozenValues = this.modelManager.duplicateItemWithCustomContent(
          {
        ...
        const frozenContentDiffers = !_tempItemWithFrozenValues.isItemContentEqualWith(
          tempServerItem
        );
        const currentContentDiffers = !itemRef.isItemContentEqualWith(
          tempServerItem
        );
        ...
        if (serverItemResponse.deleted || itemRef.deleted) {
          ...
          const isActivelyBeingEdited =
            (new Date() - itemRef.client_updated_at) / 1000 <
            IsActiveItemSecondsThreshold;
        ...
        if (duplicateLocal) {
          const localDuplicate = await this.modelManager.duplicateItemWithCustomContentAndAddAsConflict(
            {
              content: frozenContent,
              duplicateOf: itemRef
            }
        ...
        if (keepServer) {
          await this.modelManager.mapResponseItemsToLocalModelsOmittingFields(
            [serverItemResponse],
            null,
            SFModelManager.MappingSourceRemoteRetrieved
          );
  ...
  async deprecated_handleUnsavedItemsResponse(unsaved) {
    ...
    for (const mapping of unsaved) {
      const itemResponse = mapping.item;
      await SFJS.itemTransformer.decryptMultipleItems(
        [itemResponse],
        (await this.getActiveKeyInfo(SFSyncManager.KeyRequestLoadSaveAccount))
          .keys
      );
      const item = this.modelManager.findItem(itemResponse.uuid);
      ...
      if (error.tag === 'uuid_conflict') {
        ...
        const dup = await this.modelManager.createDuplicateItemFromResponseItem(
          itemResponse
        );
  ...
  */
      ...
      try {
        this.httpManager.postAuthenticatedAbsolute(
          ...
          async response => {
            ...
            const incomingItems = response.retrieved_items;
            const keys = (await this.getActiveKeyInfo(
              SFSyncManager.KeyRequestLoadSaveAccount
            )).keys;
            await SFJS.itemTransformer.decryptMultipleItems(
              incomingItems,
            ...
            options.retrievedItems = options.retrievedItems.concat(
  ...
  async resolveOutOfSync() {
    ...
    return this.stateless_downloadAllItems({
      ...
      await this.modelManager.mapResponseItemsToLocalModelsWithOptions({
        items: itemsToMap,
