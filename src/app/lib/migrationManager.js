...
import { SFAuthManager } from './authManager';
...
export class SFMigrationManager {
  constructor(modelManager, syncManager, storageManager, authManager) {
    ...
    const signInHandler = authManager.addEventHandler(event => {
    ...
    this.syncManager.addEventHandler(async (event, data) => {
      const dataLoadedEvent = event === 'local-data-loaded';
      const syncCompleteEvent = event === 'sync:completed';
      ...
      if (dataLoadedEvent || syncCompleteEvent) {
        ...
        if (this.receivedLocalDataEvent && this.receivedSyncCompletedEvent) {
          ...
          if (didReceiveSignInEvent) {
            ...
            const completedList = (await this.getCompletedMigrations()).slice();
            for (const migrationName of completedList) {
              const migration = await this.migrationForEncodedName(
                migrationName
              );
  ...
  async migrationForEncodedName(name) {
    const decoded = await this.decode(name);
  ...
  async runPendingMigrations() {
    ...
    for (const item of this.modelManager.allNondummyItems) {
      for (const migration of pending) {
  ...
  async getCompletedMigrations() {
    if (!this._completed) {
      const rawCompleted = await this.storageManager.getItem('migrations');
      if (rawCompleted) {
