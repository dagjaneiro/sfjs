...
import { SFItemHistory } from '../session_history/itemHistory';
...
export class SFHistorySession extends SFItem {
  constructor(json_obj) {
    ...
    uuids.forEach(itemUUID => {
      ...
      this.content.itemUUIDToItemHistoryMapping[itemUUID] = new SFItemHistory(
        itemHistory
      );
    });
  }
  ...
  historyForItem(item) {
    ...
    if (!history) {
      history = this.content.itemUUIDToItemHistoryMapping[
        item.uuid
      ] = new SFItemHistory();
    }
    return history;
  ...
  optimizeHistoryForItem(item) {
    ...
    var itemHistory = this.historyForItem(item);
    if (
      itemHistory.entries.length >
      SFHistorySession.LargeItemEntryAmountThreshold
    ) {
