...
export class SFItemParams {
  constructor(item, keys, auth_params) {
    ...
    if (this.keys && !this.auth_params) {
      throw new Error(
        'SFItemParams.auth_params must be supplied if supplying keys.'
      );
    }

    if (this.auth_params && !this.auth_params.version) {
      throw new Error('SFItemParams.auth_params is missing version');
    }
  }
  ...
  async __params() {
    ...
    if (!this.item.errorDecrypting) {
      ...
      if (this.keys && !doNotEncrypt) {
        var encryptedParams = await SFJS.itemTransformer.encryptItem(
          this.item,
          this.keys,
          this.auth_params
        );
        ...
        params.content = this.forExportFile
          ? this.item.createContentJSONFromProperties()
          : '000' +
            (await SFJS.crypto.base64(
              JSON.stringify(this.item.createContentJSONFromProperties())
            ));
