...
export class SFSingletonManager {
  constructor(modelManager, syncManager) {
    ...
    */
    modelManager.addModelUuidChangeObserver(
      'singleton-manager',
      (oldModel, newModel) => {
        for (const handler of this.singletonHandlers) {
          if (
            handler.singleton &&
            SFPredicate.ItemSatisfiesPredicates(newModel, handler.predicates)
          ) {
  ...
  resolveSingletons(retrievedItems, savedItems, initialLoad) {
    ...
    for (const singletonHandler of this.singletonHandlers) {
      ...
      const retrievedSingletonItems = this.modelManager.filterItemsWithPredicates(
        retrievedItems,
        predicates
      );
      ...
      const handleCreation = () => {
        if (singletonHandler.createBlock) {
          ...
          singletonHandler.createBlock(created => {
            ...
            singletonHandler.resolutionCallback &&
      ...
      const savedSingletonItemsCount = this.modelManager.filterItemsWithPredicates(
        savedItems,
        predicates
      ).length;
      ...
      if (retrievedSingletonItems.length > 0 || savedSingletonItemsCount > 0) {
        ...
        */
        const allExtantItemsMatchingPredicate = this.modelManager.itemsMatchingPredicates(
          predicates
        );
        ...
        */
        if (allExtantItemsMatchingPredicate.length >= 2) {
          ...
          const toDelete = sorted.slice(1, sorted.length);
